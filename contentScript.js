...
chrome.storage.sync.get('option', (data) => {
  if (data.option) {
    ...
    async function sendToAPI() {
      ...
      const inputText = document.getElementById("ai-selected-headline").value;
      const textareaText = document.getElementById("ai-selected-bodytext").value;
      ...
      try {
        ...
        document.getElementsByClassName('smartocto-ai-selected-option')[0].innerText = "headlines";
        ...
        const response = await fetch(apiUrl, requestOptions)
          ...
          .then(data => {
            ...
            let headlineContent = '<ul class="ai__suggestions-list">';
            for (let key in data.alternatives) {
              /* console.log(`${key}: ${data.alternatives[key]}`); */
    ...
    function compileDock(optimiseOption) {
      if ( document.getElementById('smartocto-ai-results') ) {
        ...
        tempContent += `<div id="smartocto-ai-form"><p id="smartocto-ai-instruction"></p>
      <input type="text" id="ai-selected-headline" readonly><br />
      <button id="ai-input-headline">Copy selected headline</button><br><br />
      <textarea id="ai-selected-bodytext"></textarea><br />
      <button id="ai-input-body">Copy selected bodytext</button><br><br />
      <button id="ai-generate-content">Generate output</button></div>
      `;
        //Fill with headlines
        //tempContent += '<div id="smartocto-ai-headlines"><p>One moment please, smartocto.ai is working on ' + optimiseOption + '...</p></div>';
...
`;
        ...
        document.getElementById('close-smartocto-ai-suggestions').addEventListener('click', function () {
          // Get the parent element and hide it by setting display to none
          //document.getElementsByClassName('smartocto-ai-selected-option')[0].innerText = optimiseOption;
        ...
        document.getElementById('ai-generate-content').addEventListener('click', function (e) {
          console.log('click on button: ai-input-body');
    ...
    compileDock('Headlines');

    // var div = document.createElement('div');
    // div.id = 'overlay';
    // div.innerHTML ="<iframe src='https://tentacles-staging.smartocto.com/ten/tentacles-20-desk/index.html?v=1739376873052'></iframe>"
