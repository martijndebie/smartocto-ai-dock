...
chrome.storage.sync.get(['option', 'apiToken', 'clientId', 'brandId'], (data) => {
  if (data.apiToken && data.clientId && data.brandId) {

    let hostname = document.location.hostname;
    hostname = hostname.replace('\.', '')

    localStorage.setItem('clientId_' + hostname, data.clientId);
    console.log('clientId for domein:' + hostname, data.clientId);

    localStorage.setItem('brandId_' + hostname, data.brandId);
    console.log('Injected option:', data.clientId);

    localStorage.setItem('apiToken_' + hostname, data.apiToken);
    console.log('Injected option:', data.apiToken);
    ...
    function updateDisplayedTimestamp(timestampGeneratedValue) {
      document.getElementById('smartocto-ai-timestamp').innerHTML = "Generated: " + timestampGeneratedValue;
    }
    ...
    async function sendToAPI(selectedActionType) {
      ...
      document.getElementById('smartocto-ai-output').innerHTML = "";
      document.getElementById('smartocto-ai-output').style.display = "none";
      ...
      if (selectedActionType == 'headlines') {
        ...
        try {
          payload.text = inputText + "||" + textareaText;
    ...
    function compileDock(optimiseOption) {
      if (document.getElementById('smartocto-ai-results')) {
        ...
        tempContent += '<div id="smartocto-ai-optionbox"><h3>Optimise:</h3><div class="smartocto-ai-selected-option" data-role="optimise-selected">' + optimiseOption + '</div></div>';
      ...
      `;
        ...
        tempContent += `<div id="smartocto-ai-output"></div>`;

        // Update timestamp in the dock
        let timestampGenerated = new Date();
        tempContent += `<div id="smartocto-ai-timestamp"></div>`;
